...
class DistributionLogNormal2(ScipyMixin, Distribution):
    ...
    $$
        ...
        \exp\left(-\frac{(\log y - \log \mu)^2}{2\sigma^2}\right).
    $$
    respectively
    $$
        f(y | \theta_0, \theta_1) = \frac{1}{y\theta_1\sqrt{2\pi}}
        \exp\left(-\frac{(\log y - \log \theta_0)^2}{2\theta_1^2}\right).
    $$
    ...
    parameter_support = {0: (np.nextafter(0, 1), np.inf), 1: (np.nextafter(0, 1), np.inf)}
    ...
    def initial_values(self, y: np.ndarray, param: int = 0, axis: Optional[int | None] = None) -> np.ndarray:
        log_y = np.log(y)
        if param == 0:
