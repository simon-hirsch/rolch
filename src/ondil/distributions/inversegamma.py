...
class DistributionInverseGamma(ScipyMixin, Distribution):
    ...
    def dl1_dp1(self, y: np.ndarray, theta: np.ndarray, param: int = 0) -> np.ndarray:
        ...
        if param == 1:
            term = (np.log(mu) + (alpha / (alpha + 1)) +
                    np.log(alpha + 1) - digamma(alpha) -
                    np.log(y) - (mu / y))
    ...
    def initial_values(self, y: np.ndarray, param: Optional[int] = None, axis: Optional[int] = None) -> np.ndarray:
        y = np.asarray(y)
